...
\begin{abstract}
    We propose the implementation in the \agda{}~proof assistant of a new deductive system for the modal logic $\logic$. $\logic$ extends $\axname{S4}$ with the axiom $\Diamond\Box\phi \rightarrow \Box\Diamond\phi$ and arises as the underlying logic in different fields, from mathematics to philosophy.%: it is the logic of four-dimensional Minkowski geometry under the Diodorean interpretation of modality $\Box$; it characterizes forcing over \textsf{ZFC} set theory; and, under suitable interpretations of semantic frames, it is the logic of abelian groups. Last but not least, $\logic$ is considered by epistemologists a good  logic for knowledge representation. 
 Despite its widespread applications, $\logic$ has been under-investigated (with respect to other modal systems) for several years.  In this paper we focus on the implementation of the metatheory of $\mathsf{E}_{\logic}$, a sequent calculus for $\logic$ built on previous investigations, where formulae are equipped with a \emph{position}, i.e.\ a set of uninterpreted tokens used to manage modal information. The calculus is designed to enjoy strong proof-theoretic properties, such as a direct syntactic proof of cut elimination, which in turn yields consistency of the system and the subformula property. We implement in \cubical{} both the system and the proof of the Cut-Elimination Theorem. Moreover, we propose the implementation of a constructive version of a semantics for $\logic$ based on semilattices %Despite we work on a classical modal logic, our mechanization of $\logic$ is strongly oriented to constructive mathematics. To overcome the intrinsic classical of the relational semantics of $\logic$, we formalize a finite decidable variant of the semilattice semantics (previously introduced for the natural deduction counterpart of our system).
    and we prove a constructive soundness theorem for $\mathsf{E}_{\logic}$, establishing, relying on Segerberg's finite model property, the (almost) constructive equivalence between the Hilbert-style axiomatization and the sequent calculus.
...
Despite this wide range of applications, with respect to other systems $\logic$ seems to be less investigated both from a theoretical and a implementation-oriented perspective (we discuss both some ``pen-on-paper'' deductive systems and implementations in~\ref{sec:related}). In particular, since the intuitionistic axiomatization of $\logic$ is not known, its computational aspects remain unexplored. To deepen its metatheoretical properties seems to have a twofold interesting: to advance in the knowledge of $\logic$ proof theory and, through the implementation, try to understand the classical content of the proofs, possibly moving to a purely constructive metatheory.

\subsection{Our Contribution, from theory to implementation}
In this present paper we do the first step in the direction sketched above by focusing on the implementation and building on our previous investigation in natural deduction. We propose and implement a new deductive system  for $\logic$ called $\eseq$ as a version of \emph{2-sequent}s, originally introduced in~\cite{Masini1992:TwoSequentModalities, Masini1993:TwoSequentIntuitionism} and then developed in~\cite{}. More recently, they are further expanded in~\cite{Martini2023:CutElimination}, where a extensible system able to capture all the logic between $\axname{K}$ and $\axname{S5}$ (but the intermediate systems between $\axname{S4}$ and $\axname{S5}$) is studied. We build in particular on this latter version of 2-sequents and on a natural deduction system counterpart in~\cite{Martini2024:S42NaturalDeduction}.% w.r.t. to the system $\eseq$ can be considered the sequent calculi counterpart.
The leading idea of this modal  proof-theory style is to decorate formulae with a modal information, called \emph{position}, adding a spatial extension to the proofs system.
As shown in~\cite{Martini2023:CutElimination}, by interpreting  positions as a \emph{lists} of uninterpreted token (then forcing a notion of positionality and molteplicity), one can characterize different systems by ``tuning '' some constraints on modal rules in a rule systems shared by all the logic in the framework.  According to~\cite{Martini2024:S42NaturalDeduction}, lists for positions are too structured to capture the intermediate system $\logic$, and the correct choice is to move to a notion of \emph{set-positions}, where positions  are simply set of tokens.

The sequent calculus $\eseq$ for $\logic$ we present, study and implement enjoys some strong properties. Our main results is the Cut-Elimination Theorem, fully formalized in the \agda{} proof assistant, that implies as a corollary the (syntactic proof) of the concistency of the system and the Subformula Property. We also present the implementation of a direct proof of the latter foundamental theorem.% Moreover, $\eseq$ is complete w.r.t. the Hilbert-style axiomatization of $\logic$ and it is also s
...
The use of (set-)positions to track modal information always introduces specific challenges, for example the cut elimination proof must account for token freshness, position substitution, and the interaction between eigenpositions and the Geach axiom. As a consequence, the proofs of the theorems are not a simple rework, in particular its embedding in \agda{} require a careful approach.
...
Following the approach of 2-sequents, we do not need an explicit formalization of the accessibility relation as in labelled systems~\cite{Negri:2011, Basin1998:NonClassical}: positions act as a kind of spatial coordinates that provides a ``geometrical'' information to formulae.
...
Let us now turn to the choices on the implementation side. As already emphasized, the proof theory of modal logic is far from trivial, and the correct management of modal information within proofs makes the proofs themselves highly complex.
...
We carry out our formalization in \emph{Cubical \agda}~\cite{VMA2021:CubicalAgda,AgdaTeam2025:Documentation}, an extension of \agda{} that replaces axiom-based propositional equality with a computational one grounded in cubical type theory.
This choice offers concrete advantages: function extensionality and univalence hold definitionally, quotient types (higher inductive types) are natively supported, and the \texttt{--safe} flag can be enforced throughout, ensuring no postulates compromise the formalization.
The \textsf{agda/cubical} library provides infrastructure for algebraic structures (e.g., semilattices), sorted data structures, and decidable equality, all of which we use extensively (see Section~\ref{sec:finite-semantics}).

Compared to tactic-based proof assistants such as Coq, Lean, or Isabelle, \agda{}'s functional, proof-term--oriented approach and Unicode syntax keep formalized definitions close to their mathematical presentations---particularly advantageous for a system in which the manipulation of positions plays a central role.
...
Section~\ref{sec:calculus} introduces the sequent calculus $\calculus$, including positions, position-formulae, the inference rules, and weak completeness w.r.t.\ the Hilbert-style axiomatization.
...
In this section we present the syntax, the deductive system and the Weak Completeness Theorem w.r.t.\ a standard Hilbert-style axiomatization of $\logic$.
...
A private module \texttt{G}, parameterized by the formula $A$, pre-computes the positions used in the proof: $\alpha = \varnothing$, $\beta = \{0\}$, $\gamma = \{1\}$, $\delta = \{0,1\}$.
...
A private module \texttt{N}, parameterized by a formula $A$ and a proof \texttt{pA} of $\vdash A^{\varnothing}$, computes a fresh eigentoken \texttt{xN} as the successor of the maximum token occurring in \texttt{pA} (via \texttt{maxEigenposToken}).
...
\ExecuteMetaData[EPTCS/CutElimCode.tex]{cutElim}
...
\ExecuteMetaData[EPTCS/CutElimCode.tex]{mixType}
...
Although the Subformula Property follows as a corollary of the Cut-Elimination Theorem (since cut-free proofs satisfy the property trivially), our formalization also includes a \emph{direct} proof (\texttt{SubformulaProperty'}\agdalink{S4dot2.CutElimination.SubformulaProperty.html\#SubformulaProperty\%27}) that establishes the result by induction on the derivation without appealing to cut elimination.
...
To date, we focused on syntactic aspects and structural properties of the sequent calculus $\eseq$, in particular on weak completeness with respect to the Hilbert-style axiomatization, and on the analyticity of the system by proving and implementing  the proof of the Cut-Elimination Theorem and its consequences.

In this section we address a constructive and decidable version of a suitable semantics for $\logic$ equipped with  position-style deductive system.

It is known from the literature that $\logic$ is semantically characterized by directed pre-orders, but it is also sound and complete with respect to the class of \emph{semilattices with minimum}, as shown in~\cite[Theorem~21]{Martini2024:S42NaturalDeduction}. This latter semantics, due to results by Goldblatt and Shehtmann~\cite{Goldblatt1980:Diodorean, Shehtman1983:ModalDomains} and called \emph{semilattice-models}, has been introduced to naturally interpret the notion of (set) position in the context of natural deduction.
...
In few words, given a semilattice with minimum $\langle W, \leq, \mathsf{m} \rangle$ and a valuation $V : W \to 2^{At}$, an interpretation $\rho$ maps tokens to worlds and extends to positions via $\rho(\varepsilon) = \mathsf{m}$ and $\rho(s \cup \{x\}) = \rho(x) \sqcup \rho(s)$. Semilattice with minimum are an essential mathematical tool to interpret position. Infact,  once one decides which point in the Kripke model could be uniquely associated with a set of tokens $\{x_1,\ldots,x_n\}$ by taking the supremum,
in a direct pro-order   this choice would not be unique, and in a direct pre-order the supremum of a finite set of elements might not exist. Thanks to semilattices with minimum, the problem disappears.
...
Then we build on this obstacle and move toward a fist step into a purely constructive semantics for $\logic$,  in the short-time perspective to define its intuitionistic counterpart, a still open problem (see Section~\ref{sec:conclusions_future}).

We describe now the \agda{}~formalization of a \emph{finite decidable} variant of this semantics, together with a constructive soundness theorem for $\eseq$ and the resulting equivalence with the Hilbert axiomatization $\hilbertlogic{\logic}$.

From a theoretical perspective, we rely on the  \emph{finite model property} of $\logic$, first established by Segerberg~\cite{Segerberg1971:EssayClassicalModal}.  Recall that a system $\mathcal{M}$ of modal logic is said to have the finite model property if whenever a formula $A$ is not a theorem then there exists a finite model of $\mathcal{M}$ that falsifies $A$.


The move to finite models is motivated by two considerations. First, restricting to a finite carrier $\mathrm{Fin}(n)$ makes the order relation and the evaluation function decidable, allowing the entire soundness proof to be carried out constructively (under the compiler flag \texttt{--safe}, with no postulates). Second, it avoids the universe-level complications that arise in a fully general semilattice formalization, where satisfaction must quantify over worlds of arbitrary type.

\subsection{Finite Semilattice Models in \agda}\label{ssec:finite-models}

Remember that $\mathrm{Fin}(n)$ denotes a type with $n$ elements or, in other words, elements of $\mathrm{Fin}(n)$  can be seen as natural numbers in the $I_m= \{m | m < n\}$.
...
Notice that every finite model $\mathcal{M}$ admits a canonical instance, called $\mathsf{defaultModalSemantics}$, obtained directly from the bounded iteration defining $\mathsf{eval}$.

Thanks to the purely constructive interpretation described above, we can also prove \emph{purely} constructively the soundness for (the implementation of) $\eseq$ w.r.t.\ the just defined semantics: is a sequent is derivable in the system, then it is also satisfiable.

%\subsection{Constructive Soundness}\label{ssec:constructive-soundness}


%\fbox{non abbiamo definito $\mathsf{ms}$ ma mi sembra non sia essenziale, toglierei}
...
\begin{itemize}
\item $\Box\vdash$: from $\pf{\Box A}{s}$ in $\Gamma$ and $s \subseteq t$ in the rule premise, we obtain $\mathsf{eval}(\rho\text{-pos}(\rho, s), \Box A) = \mathsf{true}$ and $\rho\text{-pos}(\rho, s) \leq \rho\text{-pos}(\rho, t)$, then apply $\mathsf{box\text{-}sem}$.
...
\noindent
The \agda{} type of the soundness theorem directly mirrors the statement above:

\ExecuteMetaData[EPTCS/CutElimCode.tex]{finiteSoundness}

\paragraph{An important remark about the constructiveness of the proof.} The decidability of satisfaction at each step is crucial: it allows the inductive argument to case-split without invoking the law of excluded middle. The proof is fully constructive and compiles under \texttt{--cubical --safe} with no postulates.

\bigskip

%Thanks to the Weak Completeness Theorem~\ref{thm:weakcompl} and Theorem~\ref{thm:finite-soundness} just proven, we can state the equivalence between our deductive system and the Hilbert-style axiomatization $\hilbertlogic{\logic}$ in Section~\ref{sec:calculus}.
...
The direction from $\hilbertlogic{\logic}$ to $\eseq$ is exactly the Weak Completeness Theorem: if $\vdash_{\hilbertlogic{\logic}} A$ then $\seq{}{A^\varepsilon}$ is provable in $\eseq$. For the converse, we rely on the well-known \emph{finite model property} (FMP) of $\logic$. Following Harrop~\cite{Harrop1958:FiniteModels}, we say that a logic has the FMP if every non-provable formula has a finite counter\-model. In the formalization, we encode this as a definition together with two classical postulates from the literature, and derive all remaining results.
...
\smallskip\noindent\textit{Justification.} Harrop~\cite{Harrop1958:FiniteModels} proves that the FMP implies decidability (Lemma~4.1). Since $\logic$ has the FMP, provability in $\hilbertlogic{\logic}$ is decidable.
\end{definition}
...
In this paper, we introduced $\mathsf{E}_{\textsf{S4.2}}$, an extended sequent calculus for the modal logic $\logic$, which enrich formulae with coordinates called positions---represented as sets of uninterpreted tokens--- to manage modal information. We implemented the system and its metatheoretical structural properties in the \agda{}~proof assistant: we prove the weak completeness w.r.t.\ a standard Hilbert-style and provide a formal syntactic proof of the cut-elimination theorem, from which the consistency of the system and the subformula property follow. %We also formalized the weak completeness of $\calculus$ with respect to the Hilbert-style axiomatization of $\logic$.
On the semantic side, we formalized a finite decidable variant of the semilattice semantics, proved a constructive soundness theorem (with no postulates, under \texttt{--safe}), and established the equivalence between provability in $\hilbertlogic{\logic}$ and provability in $\eseq$ (Section~\ref{sec:finite-semantics}).


Our investigation opens several promising research directions. As ongoing work, we are focusing on the definition of an intuitionistic version of $\logic$, denoted by $I\logic$, which constitutes a non-trivial task. On the one hand, the intuitionistic deductive system can be naturally obtained by restricting sequents to have a single formula on the right-hand side. On the other hand, a standard Hilbert-style intuitionistic axiomatization is not available in the literature and must therefore be carefully developed, particularly with regard to the elimination of classical axioms.
...
\section*{Funding}
This work was supported by the Istituto Nazionale di Alta Matematica (INdAM), through the Gruppo Nazionale per le Strutture Algebriche, Geometriche e le loro Applicazioni (GNSAGA).
